...
func (c *CAManager) secondaryInitializeIntermediateCA(provider ca.Provider, config *structs.CAConfiguration) error {
	...
	if needsNewIntermediate {
		if err := c.secondaryNewLeafSigningCert(provider, newActiveRoot); err != nil {
			return err
		}
	} else {
...
// secondaryNewLeafSigningCert should only be called while the state lock is held by
// setting the state to non-ready.
func (c *CAManager) secondaryNewLeafSigningCert(provider ca.Provider, newActiveRoot *structs.CARoot) error {
...
func (c *CAManager) RenewIntermediate(ctx context.Context, isPrimary bool) error {
	...
	if !isPrimary {
		renewalFunc = c.secondaryNewLeafSigningCert
	}
